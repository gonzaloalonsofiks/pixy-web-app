...
function Cart() {
  ...
  const {cartList, emptyCart, subtotal, removeItem, totalCart} = useCartContext()
      ...
      <div>
        ...
        <div className="cartList-container">
          <table className="cartList-table" border="2">
            ...
            {cartList.map(item => <tr  className="cartList-row" key={item.id}>
              <td><span className="cartList-remove" onClick={() => removeItem(item.id)}><AiFillDelete /> </span></td>
              <td>{item.cantidad}</td>
              <td>{item.name}</td>
              <td>$ {item.price}</td>
              <td>$ {subtotal(item.cantidad, item.price)}
              </td>
              </tr>)
              }
          </table>
